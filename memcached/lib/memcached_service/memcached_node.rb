...
class VCAP::Services::Memcached::Node
  ...
  def provision(plan, credentials = nil, version=nil)
    ...
    # raise ServiceError.new(MemcachedError::MEMCACHED_INVALID_PLAN, plan) unless plan == @plan
...
class VCAP::Services::Memcached::Node::ProvisionedService
  ...
  def connect
    ...
    # logger.debug("Connected to memcached instance: #{self[:ip]}\:#{SERVICE_PORT}")
  rescue => e
  ...
  def service_script
    # [sudo -u vcap?] memcached -m memory_size -p port_num -c connection -v -S
    cmd_components = [
      # @@run_as_user.empty? ? "" : "sudo -u #{@@run_as_user}",
