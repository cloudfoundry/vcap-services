  ...
  module Services
    module Memcached
      class MemcachedError < VCAP::Services::Base::Error::ServiceError
        ...
        MEMCACHED_INVALID_PLAN                = [31204, HTTP_INTERNAL, "Invalid plan: %s"]
        MEMCACHED_CLEANUP_INSTANCE_FAILED     = [31205, HTTP_INTERNAL, "Could not cleanup instance, the reasons: %s"]
        MEMCACHED_CONNECT_INSTANCE_FAILED     = [31206, HTTP_INTERNAL, "Could not connect memcached instance"]
        MEMCACHED_SET_INSTANCE_PASS_FAILED    = [31207, HTTP_INTERNAL, "Could not set memcached instance password"]
        MEMCACHED_RESTORE_FILE_NOT_FOUND      = [31208, HTTP_INTERNAL, "Could not find memcached restore data file %s"]
      end
    end
  end
end
