...
def get_node_config()
  ...
  # TODO: we read the capacity and port range from config file (from vcap/dev_setup/cookbooks/memcached/templates/memcached_node.yml.erb
  # Since that change goes in a separate repo, putting the capacity and port-range here manually for now. 
  ...
  options = {
    ...
    :capacity => 50, #parse_property(config, "capacity", Integer),
    :node_id => parse_property(config, "node_id", String),
    :port_range => Range.new(5000, 25000), #parse_property(config, "port_range", Range),
