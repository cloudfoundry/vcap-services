...
describe VCAP::Services::Memcached::Node do
  ...
  describe 'Node.initialize' do
    ...
    it "should set up an available capacity" do
      @node.available_capacity.should be @options[:capacity]
    end
  ...
  describe "Node.start_db" do
    it "should fail when set local db with non-existing file argument" do
      @node.db_local_options = "sqlite3:/non_existing/non-existing.db"
      thrown = nil
      begin
        @node.start_db
      rescue => e
      ...
      @node.db_local_options = @options[:local_db]
    end
  ...
  describe 'Node.announcement' do
    ...
    it "should send available_capacity in announce message" do
      @node.announcement[:available_capacity].should == @node.available_capacity
    end
  end
  ...
  describe "Node.provision" do
    ...
    it "should decrease available capacity when finish a provision" do
      (@old_capacity - @node.available_capacity).should == @capacity_unit
    end
  ...
  describe "Node.unprovision" do
    ...
    it "should increase available capacity when finish an unprovision" do
      (@node.available_capacity - @old_capacity).should == @capacity_unit
    end
  ...
  describe "Node.memory_for_instance" do
    it "should return  by the plan" do
      instance = VCAP::Services::Memcached::Node::ProvisionedService.new
      instance.plan = :free
