    ...
    module Marketplace
      module Appdirect
        class AppdirectMarketplace < VCAP::Services::Marketplace::Base
          ...
          def load_mapped_name_and_provider(key)
            @mapping[key.to_sym] if @mapping.keys.include?(key.to_sym)

            # If mapping is not defined, then raise
            raise "Requested mapping for unknown label: #{name} / provider: #{provider}"
          end
