      ...
      module Appdirect
        class AppdirectMarketplace < VCAP::Services::Marketplace::Base
          ...
          def generate_cc_advertise_request(name, bsvc, active = true)
            ...
            provider = bsvc["provider"] || name # We'll use the service name as provider unless appdirect sends otherwise
          ...
          def offering_disabled?(id, offerings_list)
            ...
            @logger.info("Offering: #{id} - Present in offering list: #{offerings_list.include?(id)}, whitelist: #{(@whitelist.nil? || @whitelist.include?(id))}")
            !(offerings_list.include?(id) && (@whitelist.nil? || @whitelist.include?(id)))
          end
          ...
          def provision_service(request_body)
            ...
            id = @service_id_map[id] if @service_id_map.keys.include?(id)
            @logger.info("Provision request for label=#{request.label} (id=#{id}) plan=#{request.plan}, version=#{request.version}")
