      ...
      class MarketplaceAsyncServiceGatewayV2 < VCAP::Services::BaseAsynchronousServiceGateway
        ...
        helpers do
          ...
          def is_a_valid_auth_token(token)
            ...
            @logger.debug("Token: #{token} is #{valid ? "valid" : "invalid"}")
          end
        ...
        def validate_incoming_request
          ...
          if "marketplace" == request.path_info.split('/')[1] || request.path_info == "/"
            # old fashioned placeholder token to be used by specs
            unless auth_token && auth_token == PLACEHOLDER_TOKEN
              error_msg = ServiceError.new(ServiceError::NOT_AUTHORIZED).to_hash
              @logger.error("ERROR: #{error_msg.inspect}")
              abort_request(error_msg)
            end
          else
