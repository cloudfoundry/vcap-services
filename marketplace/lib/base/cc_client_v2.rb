  ...
  module Services
    module Marketplace
      class CCNGClient
          ...
          def service_plans_uri(service_guid)
            "#{@cld_ctrl_uri}/v2/services/#{service_guid}/serivce_plans"
          end
          ...
          def advertise_service(offering, guid)
            ...
            if http.error.empty?
              if (200..300) === http.response_header.status
          ...
          def add_plan(plan)
            @logger.info("Add new plan: #{plan.inspect} via #{@service_plans_uri}")

            req = create_http_request(Yajl::Encoder.encode(plan))

            f = Fiber.current

            http = EM::HttpRequest.new(@service_plans_uri).post(req)
            http.callback { f.resume(http) }
            http.errback { f.resume(http) }
            ...
            if http.error.empty?
              if (200..300) === http.response_header.status
                @logger.info("Successfully added service plan: #{plan.inspect}")
                return true
              else
                @logger.warn("Failed to add plan: #{plan.inspect}, status=#{http.response_header.status}")
              end
            else
              @logger.warn("Failed to add plan: #{plan.inspect}: #{http.error}")
            end

            return false
          end

          def update_plan(plan_guid, plan)
            url = "#{@service_plans_uri}/#{plan_guid}"
            @logger.info("Update plan: #{plan_guid} to #{plan.inspect}, url: #{url}")

            req = create_http_request(Yajl::Encoder.encode(plan))

            f = Fiber.current

            http = EM::HttpRequest.new(url).put(req)
            http.callback { f.resume(http) }
            http.errback { f.resume(http) }
            ...
            if http.error.empty?
              if (200..300) === http.response_header.status
                @logger.info("Successfully updated service plan: #{plan.inspect}")
                return true
              else
                @logger.warn("Failed to update plan: #{plan.inspect}, status=#{http.response_header.status}")
              end
            else
              @logger.warn("Failed to update plan: #{plan.inspect} due to: #{http.error}")
            end
