    ...
    module Marketplace
      class MarketplaceAsyncServiceGateway < VCAP::Services::AsynchronousServiceGateway
        ...
        def load_cc_client(opts)
          ...
          cc_version   = opts[:cc_version] || "v1"

          @logger.info("CloudController API version: #{cc_version}")
          if cc_version == "v1"
            VCAP::Services::Marketplace::CloudControllerClient.new(
            {
              :service_list_uri => service_list_uri,
            ...
            raise "Cloud Controller client unknown for version: #{cc_version}"
          end
        end
