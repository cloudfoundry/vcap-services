...
func (w *Warden) Block(id string) (ok bool) {
	return LimitWardenBandwidth(w.Bin, id, w.BlockRate, w.BlockBurst)
}
func (w *Warden) Unblock(id string) (ok bool) {
...
func (w *Warden) GetRate(id string) (rate, burst int64, err error) {
	...
	info, err := cmd.Output()
	if err != nil {
		logger.Logger().Errorf("Run warden info error [%s]", err)
		return
	}
...
func LimitWardenBandwidth(bin, id string, rate int64, burst int64) bool {
	cmdStr := fmt.Sprintf(CMDTP, bin, id, rate, burst)
	cmd := exec.Command(strings.Split(cmdStr, " ")[0], strings.Split(cmdStr, " ")[1:]...)
	_, err := cmd.Output()
	if err != nil {
		logger.Logger().Errorf("Run warden limit bandwidth error [%s]", err)
		return false
	}
	return true
}
