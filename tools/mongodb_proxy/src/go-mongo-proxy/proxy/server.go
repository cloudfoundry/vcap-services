			...
			for i := 0; i < nfds; i++ {
				...
				if netio.ProxyNetIsProxyServer(fd) {
					...
					if err != nil {
                        ...
                        logger.Info("Succeed to establish bridge for client [%s:%d].", ipaddr, port)
                    }
				} else {
					...
					if event & syscall.EPOLLIN != 0 {
						...
						case SESSION_EOF:
                            ...
                            if sa != nil {
                                ipaddr, port := parse_sockaddr(sa)
                                logger.Error("One side [%s:%d] close the session.", ipaddr, port)
                            }
						}
					...
					if event & syscall.EPOLLRDHUP != 0 {
                        ...
                        if sa != nil {
                            ipaddr, port := parse_sockaddr(sa)
                            logger.Info("shutdown connection with [%s:%d].", ipaddr, port)
						    netio.ProxyNetClosePeers(fd)
                        }
					}
