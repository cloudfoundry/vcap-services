...
type Session interface {
	Reset(*net.TCPConn, *net.UnixConn, Filter)
	GetSid() int32
	Process()
	WaitForFinish()
}
...
type SessionManager interface {
	NewSession(*net.TCPConn, *net.UnixConn, Filter) Session
	WaitAllFinish()
	MarkIdle(Session)
}
...
func (session *ProxySessionImpl) Reset(clientfd *net.TCPConn, serverfd *net.UnixConn, f Filter) {
	...
	for {
		...
		if err != nil {
			if err == io.ErrUnexpectedEOF {
				logger.Debug("UnixSocket session with mongodb server encounter unexpected EOF: [%v].", err)
				break
			} else if err.Error() == errClosing.Error() {
				logger.Debug("UnixSocket session is closed: [%v].", err)
				break
			}
			logger.Error("UnixSocket write to server error: [%v].", err)
			continue
		}
...
func (session *ProxySessionImpl) ForwardServerMsg() {
	...
	for {
		...
		if err != nil {
			if err == io.EOF {
				logger.Debug("UnixSocket session with mongodb server will be closed soon.")
				break
			} else if err.Error() == errClosing.Error() {
				logger.Debug("UnixSocket session is closed: [%v].", err)
				break
			}
			logger.Error("UnixSocket read from server error: [%v].", err)
			continue
		}
...
func (manager *ProxySessionManagerImpl) NewSession(clientfd *net.TCPConn, serverfd *net.UnixConn, f Filter) Session {
	var session Session
	var sid int32
...
func (manager *ProxySessionManagerImpl) SpawnSession(clientfd *net.TCPConn, serverfd *net.UnixConn, f Filter) Session {
	var sid int32
