...
class VCAP::Services::ElasticSearch::Node
  ...
  def initialize(options)
    ...
    @elasticsearch_plugins_path = options[:elasticsearch_plugins_path]
  ...
  def varz_details
    ...
    begin
      ProvisionedService.all.each do |instance|
        provisioned_instances[instance.name.to_sym] = elasticsearch_get_status(instance)
      end
    rescue => e
  ...
  def start_instance(provisioned_service)
    ...
    config_file = get_config_path(dir)
  ...
  def elasticsearch_get_status(instance)
  ...
  def setup_server(dir, provisioned_service)
    ...
    memory = @max_memory
    plugins_dir = @elasticsearch_plugins_dir

    File.open(get_config_path(dir), "w") { |f| f.write(@config_template.result(binding)) }
  ...
  def cleanup_service(provisioned_service)
    ...
    EM.defer { FileUtils.rm_rf(File.join(@base_dir, provisioned_service.name)) }
  ...
  def get_config_path(dir)
    File.join(get_config_dir(dir), 'elasticsearch.yml')
  end

  def get_config_dir(dir)
