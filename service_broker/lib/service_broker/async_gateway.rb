  ...
  def setup(opts)
    ...
    f = Fiber.new do
      begin
        ...
        @logger.debug("Retrieve currently registered brokered services")
        # TODO: This needs further treatment, for now, this call is effectively a no-op
        @catalog_manager.load_registered_services_from_cc
  ...
  helpers do
    ...
    def advertise_brokered_service(request)
      ...
      request.options.each do |opt|
        ...
        provider = name # TODO: add provider, until then use name as provider
    ...
    def delete_brokered_service(label)
      ...
      bsvcs.each do |bsvc|
        ...
        name, version = VCAP::Services::Api::Util.parse_label(bsvc.label)
        provider = bsvc.name # TODO: Add provider support
