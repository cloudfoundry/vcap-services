...
class VCAP::Services::Rabbit::Node
  ...
  def initialize(options)
    ...
    @rabbitmq_start_timeout = @options[:rabbitmq_start_timeout] || 4
  ...
  def destroy_instance(instance)
    # Here need check whether the object is in local db or not,
    # otherwise the destory operation will persist the object from memory to db without delete it,
    # the behavior of datamapper is doing persistent work at the end of each save/update/destroy API
    db_instance = get_instance(instance.name)
    if db_instance
      raise RabbitError.new(RabbitError::RABBIT_DESTORY_INSTANCE_FAILED, instance.inspect) unless db_instance.destroy
    end
  end
