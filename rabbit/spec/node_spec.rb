...
module VCAP
  module Services
    module Rabbit
      class Node
         attr_reader :local_ip, :base_dir, :node_id, :available_memory, :max_memory, :max_clients, :rabbitmq_server
...
describe VCAP::Services::Rabbit::Node do
  ...
  describe "Node.initialize" do
    it "should set up node configuration" do
      ...
      @node.available_memory.should == @options[:available_memory]
      @node.max_memory.should == @options[:max_memory]
  ...
  describe "Node.start_provisioned_instances" do
    it "should check whether provisioned instance is running or not" do
      @node.start_instance(@instance)
      @instance.pid = @node.instance_pid(@instance.name)
  ...
  describe "Node.migration" do
    ...
    it "should not access rabbitmq server after disable the instance" do
      ...
      @binding_credentials_list.each do |credentials|
        expect {@node.get_permissions(credentials["vhost"], credentials["user"])}.should raise_error(VCAP::Services::Rabbit::RabbitError)
      end
    end
    ...
    it "should import db file from right location after import instance" do
      @node.unprovision(@instance_credentials["name"], @binding_credentials_list)
      sleep 1
      new_credentials = @node.import_instance(@instance_credentials, @binding_credentials_map, @dump_dir, :free)
      sleep 1
      @instance_credentials["port"] = new_credentials["port"]
      @binding_credentials_list.each do |credentials|
        credentials["port"] = new_credentials["port"]
      end
