    ...
    def initialize(id, score)
      ...
      @nats = NATS.connect(:uri => BaseTests::Options::NATS_URI) {
        ...
        @nats.subscribe("#{service_name}.check_orphan") do |msg, reply|
          ...
          response.orphan_ins_hash={}
          response.orphan_binding_hash={}
          response.orphan_ins_hash["#{node_id}"] = (1..@id).to_a
          response.orphan_binding_hash["#{node_id}"] = (1..@id).to_a
          response.success = true
...
class AsyncGatewayTests
  ...
  class MockGateway
    ...
    def send_purge_orphan_request
      msg = Yajl::Encoder.encode({
        :orphan_ins_hash => TEST_PURGE_INS_HASH,
        :orphan_binding_hash => TEST_PURGE_BIND_HASH
      })
