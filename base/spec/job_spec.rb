...
describe VCAP::Services::Lock do
  ...
  before :each do
    ...
    @stored_value = nil
    VCAP::Services::Lock.redis = @redis
  ...
  it "should acquire and release a lock" do
    ...
    lock = VCAP::Services::Lock.new(@name,:timeout => @timeout, :expiration => @expiration)
  ...
  it "should not let two clients acquire the same lock at the same time" do
    ...
    lock_a = VCAP::Services::Lock.new(@name,:timeout => @timeout, :expiration => @expiration, :logger => @logger)
    lock_b = VCAP::Services::Lock.new(@name,:timeout => 0.1, :logger => @logger)

    lock_a_ran = false
    lock_b_ran = false
    lock_a.lock do
      lock_a_ran = true
      expect{lock_b.lock{ lock_b_ran = true } }.should raise_error(VCAP::Services::Lock::TimeoutError)
    end
  ...
  it "should acquire an expired lock" do
    ...
    lock = VCAP::Services::Lock.new(@name,:timeout => @timeout, :expiration => @expiration, :logger => @logger)
