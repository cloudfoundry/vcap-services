...
module VCAP::Services::Base::Warden
  ...
  module ClassMethods
    ...
    attr_reader :base_dir, :log_dir, :image_dir, :max_db_size
  end
  ...
  def map_port(src_port, dest_ip, dest_port)
    ...
    5.times do
      ret = self.class.sh("iptables -t nat -A PREROUTING #{rule.join(" ")}", :raise => false)
      logger.warn("cmd \"iptables -t nat -A PREROUTING #{rule.join(" ")}\" invalid") if ret == 2
      break unless ret == 1
      sleep 0.2
    end
  end
  ...
  def unmap_port(src_port, dest_ip, dest_port)
    rule = [ "--protocol tcp",
             ...
             "--to-destination #{dest_ip}:#{dest_port}" ]
    5.times do
      ret = self.class.sh("iptables -t nat -D PREROUTING #{rule.join(" ")}", :raise => false)
      logger.warn("cmd \"iptables -t nat -D PREROUTING #{rule.join(" ")}\" invalid") if ret == 2
      break unless ret == 1
      sleep 0.2
    end
  end
