...
module VCAP::Services::Base::Warden
  ...
  def loop_setdown
    self.class.sh "umount #{base_dir}" if self.class.quota
  end

  def loop_setup
    self.class.sh "mount -n -o loop #{image_file} #{base_dir}" if self.class.quota
  end
  ...
  def image_file?
    return File.exists?(image_file) if self.class.quota
    false
  end
