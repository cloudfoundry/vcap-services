...
class VCAP::Services::Base::Node < VCAP::Services::Base::Base
  ...
  def initialize(options)
    ...
    @op_time_limit = options[:op_time_limit] || 5
  ...
  def timing_exec(time_limit, rollback=nil, &blk)
  ...
  def on_provision(msg, reply)
    ...
    rollback = lambda do |res|
      @logger.error("#{service_description}: Provision takes too long. Rollback for #{msg}")
  ...
  def on_bind(msg, reply)
    ...
    rollback = lambda do |res|
      @logger.error("#{service_description}: Binding takes too long. Rollback for #{msg}")
