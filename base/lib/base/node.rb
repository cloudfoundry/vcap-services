...
class VCAP::Services::Base::Node < VCAP::Services::Base::Base
  ...
  def on_check_orphan(msg, reply)
    @logger.debug("#{service_description}: handles for checking orphan #{msg}" )
    response = CheckOrphanResponse.new
    request = CheckOrphanRequest.decode(msg)
    check_orphan(request.handles)
    response.orphan_ins_hash = @orphan_ins_hash
    response.orphan_binding_hash = @orphan_binding_hash
  ...
  def check_orphan(handles)
    ...
    @logger.debug("Find orphan instance #{oi_list.inspect}" )

    live_bind_list=all_bindings_list
    orphan_binding_hash = {}
    ob_list = []
    live_bind_list.each do |credential|
      ob_list << credential unless handles.index{|h| h["credentials"]["username"] == credential["username"]}
    end
    @logger.debug("Find orphan binding #{ob_list.inspect}")
  ...
  def on_purge_orphan(msg, reply)
    @logger.debug("#{service_description}: Message for purging orphan #{msg}" )
    response = SimpleResponse.new
  ...
  def purge_orphan(oi_list,ob_list)
    ...
    oi_list.each do |ins|
      ...
      @logger.debug("Unprovision instance #{ins} and its bindings #{bindings.inspect}")
    ...
    ob_list.each do |credential|
      ...
      ret=false
      @logger.debug("Error on purge orphan: #{e}")
    end
    ret
  end
