...
class VCAP::Services::Base::Provisioner < VCAP::Services::Base::Base
  ...
  def init_service_extensions
    ...
    @plan_mgmt.each do |plan, value|
      ...
      @extensions[plan] ||= {}
      @extensions[plan][:snapshot] = true if lifecycle.has_key? :snapshot
  ...
  def before_snapshot_apis service_id, *args, &blk
    puts "service_id=#{service_id}, args=#{args}, blk=#{blk}"
  ...
  def before_job_apis service_id, *args
    raise "service_id can't be nil" unless service_id
  ...
  def extensions_enabled?(plan, extension)
    raise ServiceError.new(ServiceError::EXTENSION_NOT_IMPL, extension) unless @extensions[plan.to_sym][extension.to_sym]
    true
  end
