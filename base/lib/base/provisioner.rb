...
class VCAP::Services::Base::Provisioner < VCAP::Services::Base::Base
  ...
  def reset_orphan_stat
    @staging_orphan_instances = Hash.new { |h,k| h[k] = [] }
    @staging_orphan_bindings = Hash.new { |h,k| h[k] = [] }
    @final_orphan_instances = Hash.new { |h,k| h[k] = [] }
    @final_orphan_bindings = Hash.new { |h,k| h[k] = [] }
  end
  ...
  def varz_details()
    ...
    varz = {
      :nodes => @nodes,
      :prov_svcs => svcs,
      :orphan_instances => @final_orphan_instances,
      :orphan_bindings => @final_orphan_bindings
    }
    return varz
  rescue => e
    @logger.warn(e)
