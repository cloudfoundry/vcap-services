...
class VCAP::Services::Base::Provisioner < VCAP::Services::Base::Base
    ...
    if best_node && ( @allow_over_provisioning || node_score(best_node) > 0 )
      ...
      subscription =
        @node_nats.request(
          ...
          if response.success
            @logger.debug("Successfully provision response:[#{response.credentials}]")
            # credentials is no necessary in cache
  ...
  # Parse the raw message and decod servic message object according to input type.
  def parse_node_result(raw, service_msg)
    req = VCAP::Services::Message::ProvisionResponse.decode(raw)
    @logger.warn("Decoded request #{req}")
    if req.success
      req.response = service_msg.from_decoded_json(req.response)
    else
      req.error =  req.response
      req.response = nil
    end
  end
