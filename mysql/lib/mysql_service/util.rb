    ...
    module Mysql
      module Util
        ...
        def handle_discarded_routines(db_name, connection)
          begin
            res = connection.query("select * from mysql.proc where db='#{db_name}' and  security_type ='definer' and (type='PROCEDURE' OR type='FUNCTION') and definer NOT IN (select concat(user, '@', host) as valid_definer from mysql.user)")
            invalid_definers = []
