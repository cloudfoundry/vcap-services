...
class VCAP::Services::Serialization::Server < Sinatra::Base
  ...
  def initialize(opts)
    ...
    @upload_token_secret = opts[:upload_token_secret] || "dlfoosecret"
    ...
    @cc_req_hdrs = {
      VCAP::Services::Api::SDS_UPLOAD_TOKEN_HEADER => @upload_token_secret,
  ...
  def generate_file_token(service, service_id, file_name, length=12)
    ...
    appendix=Digest::MD5.hexdigest(@upload_token_secret+service+service_id+file_name+(Time.now.to_i.to_s))
    return prefix+appendix
  end

  def generate_download_url(service, service_id, token)
    url = "http://#{@external_uri}/serialized/#{service}/#{service_id}/serialized/file?token=#{token}"
  end

  def register_file(service, service_id, token, file_path)
    @redis.hset(redis_file_key(service, service_id), token, {"file" => file_path, "time" => Time.now.to_i}.to_json )
  end
  ...
  def authorized?
    request_header(VCAP::Services::Api::SDS_UPLOAD_TOKEN_HEADER) == @upload_token_secret
  end
  ...
  put "/serialized/:service/:service_id/serialized/data" do
    ...
    begin
      ...
      loop {
        ...
        # mv the file to the upload file
        new_file_path = upload_file_path(service, service_id, file_token)
      ...
      unless file_token && register_file(service, service_id, file_token, new_file_path)
