...
describe "blob_node bind" do
  ...
  it "should have valid response" do
    ...
    @resp['meta_port'].should_not be_nil
  ...
  it "should not allow unauthorized user to access the instance meta" do
    EM.run do
      conn = Mongo::Connection.new('localhost', @resp['meta_port']).db(@resp['db'])
      begin
  ...
  it "should not allow valid user with empty password to access the instance meta" do
    EM.run do
      conn = Mongo::Connection.new('localhost', @resp['meta_port']).db(@resp['db'])
      begin
        coll = conn.collection('mongo_unit_test')
        auth = conn.authenticate(@bind_resp['username'], '')
  ...
  it "should not allow user to access the instance meta after unbind" do
    EM.run do
      begin
        conn = Mongo::Connection.new('localhost', @resp['meta_port']).db(@resp['db'])
