...
def delete_admin(options)
  db = Mongo::Connection.new('127.0.0.1', options['meta_port']).db(options['db'])
  auth = db.authenticate(options['username'], options['password'])
  db.remove_user('admin')

  db = Mongo::Connection.new('127.0.0.1', options['meta_port']).db('admin')
  service = VCAP::Services::Blob::Node::ProvisionedService.get(options['name'])
