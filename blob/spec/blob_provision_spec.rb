...
describe "blob_node provision" do
  ...
  it "should be able to create bucket" do
    ...
    EM.run do
      begin
        EM.add_timer(0) { rc = AWS::S3::Bucket.create 'blob_unit_test' }
        EM.add_timer(1) { response = AWS::S3::Bucket.find 'blob_unit_test' }
  ...
  it "should be able to alert when creating object against a non-existent bucket" do
    ...
    EM.run do
        EM.add_timer(0) { 
          begin
            AWS::S3::S3Object.create('file1', 'hello world!', 'blob_unit_test2') 
          rescue => e
            err = e
          end
        } 
  ...
  it "should be able to create object against an existent bucket" do
    ...
    EM.run do
      begin
        EM.add_timer(0) { AWS::S3::S3Object.create('file1','hello world!', 'blob_unit_test') } 
        EM.add_timer(1) { response = AWS::S3::S3Object.value 'file1','blob_unit_test' }
        EM.add_timer(2) { EM.stop }
      rescue Exception => e
        @logger.debug e
      end
  ...
  it "should be able to alert when deleting a non-existent object" do
    ...
    EM.run do
        EM.add_timer(0) { 
          begin
            response = AWS::S3::S3Object.delete 'file2', 'blob_unit_test'
          rescue => e
            err = e
          end 
        }
  ...
  it "should be able to delete an existent object" do
    ...
    EM.run do
      begin
        EM.add_timer(0) { AWS::S3::S3Object.delete 'file1','blob_unit_test' }
        EM.add_timer(1) { response = AWS::S3::Bucket.find('blob_unit_test').size }
        EM.add_timer(2) { EM.stop }
      rescue Exception => e
        @logger.debug e
      end
  ...
  it "should keep the result after node restart" do
    ...
    response = AWS::S3::Bucket.find 'blob_unit_test'
    response.should_not be_nil
    response.size.should == 0
  end
  ...
  it "should be able to delete an empty bucket" do
    EM.run do
      begin
        AWS::S3::Bucket.delete 'blob_unit_test' 
      rescue Exception => e
        @logger.debug e
      end
      e.should be_nil
    ...
    EM.run do
      begin
        AWS::S3::Bucket.find 'blob_unit_test'
      rescue => e
      end
      e.should_not be_nil
      EM.stop
