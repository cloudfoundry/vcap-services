...
# start blob instance and its meta instance, meta first, then blob service!
  ...
  def shutdown
    ...
    ProvisionedService.all.each { |provisioned_service|
      @logger.debug("Try to terminate blobd pid:#{provisioned_service.pid} and meta pid:#{provisioned_service.meta_pid}")
  ...
  def provision(plan, credential = nil)
    ...
    response = {
      ...
      "meta_port" => provisioned_service.meta_port,
  ...
  def cleanup_service(provisioned_service)
    ...
    provisioned_service.kill if provisioned_service.running? || provisioned_service.meta_running?
  ...
  def restore(instance_id, backup_file)
    ...
    # Run blobrestore
    command = "#{@blobrestore_path} -u #{username} -p#{password} --port #{port} #{backup_file}"
    output = `#{command}`
    res = $?.success?
    @logger.debug(output)
    raise 'blobrestore failed' unless res
    true
  ensure
    conn.close if conn
  end
  ...
  def dump_instance(service_credential, binding_credentials, dump_dir)
    ...
    FileUtils.cp_r(File.join(from_dir, '.'), dump_dir)#this line will copy all blob files as well
    true
  rescue => e
    @logger.warn(e)
    nil
  ...
  def import_instance(service_credential, binding_credentials, dump_dir, plan)
    ...
    FileUtils.cp_r(File.join(dump_dir, '.'), to_dir)#this line will copy all blob files as well
    true
  rescue => e
    @logger.warn(e)
    nil
  ...
  def enable_instance(service_credential, binding_credentials)
    ...
    provisioned_service.meta_memory   = stored_service.meta_memory
  ...
  def varz_details
    ...
    # Get blob db.stats and db.serverStatus
    stats = []
    ProvisionedService.all.each do |provisioned_service|
      stat = {}
      overall_stats = blob_overall_stats({
        :port      => provisioned_service.meta_port,
        :name      => provisioned_service.name,
        :admin     => provisioned_service.admin,
        :adminpass => provisioned_service.adminpass
  ...
  def start_instance(provisioned_service)
    ...
    if pid
      ...
      File.open(config_path, "w") {|f| f.write(config.gsub("=>",":"))}
  ...
  def blob_overall_stats(options)
    ...
    # The following command is not documented in blob's official doc.
