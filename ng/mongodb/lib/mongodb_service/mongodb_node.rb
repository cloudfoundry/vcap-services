...
class VCAP::Services::MongoDB::Node
  ...
  def provision(plan, credential = nil, version = nil)
    ...
    raise ServiceError.new(MongoDBError::MONGODB_START_INSTANCE_TIMEOUT, p_service.name) if wait_service_start(p_service) == false
  ...
  def is_service_started(instance)
    return instance.server_status == "ok" ? true : false
  end
