...
class VCAP::Services::VBlob::Node::ProvisionedService
  ...
  def start_options
    options = super
    options[:pre_start_script] = {:script => "#{File.join(script_dir, "pre_service_start.sh")} #{base_dir} #{log_dir} #{tmp_dir}", :use_root => true}
    options[:start_script] = {:script => "#{service_script} start #{node_dir} #{bin_dir} #{base_dir} #{log_dir}", :use_spawn => true}
    options[:service_port] = service_port
    options[:bind_dirs] << {:src => tmp_dir}
  ...
  def node_dir
    File.join(self.class.bin_dir, "node")
  end

  def bin_dir
    File.join(self.class.bin_dir, "vblob")
  end
