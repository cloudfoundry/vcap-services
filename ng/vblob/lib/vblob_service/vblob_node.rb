...
class VCAP::Services::VBlob::Node
  ...
  def provision(plan, credential = nil, version=nil)
    ...
    additional_binds = [{
      :src_path => provisioned_service.tmp_dir,
      :dst_path => "/store/tmp",
      :mode     => Warden::Protocol::CreateRequest::BindMount::Mode::RW, }]
    provisioned_service.run(additional_binds)
  ...
  def enable_instance(service_credential, binding_credentials)
    ...
    additional_binds = [{
      ...
      :dst_path => "/store/tmp",
      :mode     => Warden::Protocol::CreateRequest::BindMount::Mode::RW, }]
    provisioned_service.run(additional_binds) unless provisioned_service.running?
    raise VBlobError.new(VBlobError::VBLOB_START_INSTANCE_ERROR) if wait_service_start(provisioned_service) == false
    true
  rescue => e
    @logger.warn(e)
