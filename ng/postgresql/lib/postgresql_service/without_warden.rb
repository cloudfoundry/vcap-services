...
module VCAP::Services::Postgresql::WithoutWarden
  ...
  def db_overhead(name)
    ...
    res = fetch_global_connection(name).query("select ((sum(pg_database_size(datname)) + avg(pg_tablespace_size('pg_global')))/#{divider}) as avg_overhead from pg_database where datname in ('#{@sys_dbs.join('\', \'')}');")
    res.each do |x|
