    ...
    module Postgresql
      ...
      module Util
        ...
        def postgresql_connect(host, user, password, port, database, opts={})
          ...
          try_num.times do
            begin
              (!quiet) && @logger.info("PostgreSQL connect: #{host}, #{port}, #{user}, #{password}, #{database} (fail_with_nil: #{fail_with_nil})")
              conn_opts = {
                :host => host,
                :port => port,
                :options => nil,
              ...
              (!quiet) && @logger.info("Connected PostgreSQL server - version: #{version}")
              return conn
            rescue => e
              (!quiet) && @logger.error("PostgreSQL connection attempt failed: #{host} #{port} #{database} #{user} #{password}")
              sleep(exception_sleep) if exception_sleep > 0
            end
          end

          if fail_with_nil
            (!quiet) && @logger.warn("PostgreSQL connection unrecoverable")
            return nil
          else
