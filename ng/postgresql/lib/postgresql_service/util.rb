    ...
    module Postgresql
      ...
      module Util
        ...
        def kill_long_queries_internal(connection, super_user, max_long_query)
          ...
          begin
            process_list = connection.query("select * from (select procpid, datname, query_start, usename,
            ...
            process_list.each do |proc|
              unless is_default_bind_user(proc["usename"])
                # Cancel the exact query when exceeding the query time limitation terminate the connection
        ...
        def kill_long_transaction_internal(connection, super_user, max_long_tx)
          ...
          begin
            process_list = connection.query("select * from (select procpid, datname, xact_start, usename,
