      ...
      module Util
        ...
        class DBconn
          ...
          def initialize(conn_opts)
            ...
            @conn = async? ? self.class.async_connect(valid_conn_opts) : self.class.connect(valid_conn_opts)
            if @conn
            ...
            def async_connect(conn_opts)
              ...
              while status != PGconn::PGRES_POLLING_OK && status != PGconn::PGRES_POLLING_FAILED
                ...
                when PGconn::PGRES_POLLING_READING
                  raise PGError, "Async connect timed out when reading" unless IO.select( [socket], [], [], select_timeout )
                when PGconn::PGRES_POLLING_WRITING
                  raise PGError, "Async connect timed out when writing" unless IO.select( [], [socket], [], select_timeout )
                end
                status = conn.connect_poll
              end
