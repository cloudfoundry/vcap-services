...
class VCAP::Services::Postgresql::Node
  ...
  def dump_instance(prov_cred, binding_creds, dump_file_path)
    ...
    raise "Failed to dump #{name}: cmd:#{cmd}, stdout:#{o}, stderr:#{e}, status:#{s}" unless dump_database(
      name, host, port, user, passwd, dump_file, :dump_bin => dump_bin)
  ...
  def import_instance(prov_cred, binding_creds_hash, dump_file_path, plan)
    ...
    restore_database(name, host, port, user, passwd, import_file, :restore_bin => restore_bin)
  rescue => e
