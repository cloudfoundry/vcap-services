...
class VCAP::Services::Mysql::Node::WardenProvisionedService
  ...
  ["start", "stop"].each do |op|
    define_method "#{op}_script".to_sym do
      case version
      when "5.5"
        "#{service_script} #{op} 55 #{bin_dir} /var/vcap/sys/run/mysqld /var/vcap/store/mysql /var/vcap/sys/log/mysql"
      else
        "#{service_script} #{op} '' #{bin_dir} /var/vcap/sys/run/mysqld /var/vcap/store/mysql /var/vcap/sys/log/mysql"
      end
    end
  end
