...
class VCAP::Services::Mysql::Node::WardenProvisionedService
  ...
  ["start", "stop"].each do |op|
    define_method "#{op}_script".to_sym do
      case version
      when "5.5"
        "#{script_dir}/warden_service_ctl #{op} 55 #{bin_dir} /var/vcap/sys/run/mysqld /var/vcap/store/mysql /var/vcap/sys/log/mysql #{conf_dir}"
      else
        "#{script_dir}/warden_service_ctl #{op} '' #{bin_dir} /var/vcap/sys/run/mysqld /var/vcap/store/mysql /var/vcap/sys/log/mysql #{conf_dir}"
      end
    end
  end
