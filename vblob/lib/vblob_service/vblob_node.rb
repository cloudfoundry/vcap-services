  ...
  def pre_send_announcement
    @capacity_lock.synchronize do
      ProvisionedService.all.each do |provisioned_service|
        ...
        unless service_exists?(provisioned_service)
...
class VCAP::Services::VBlob::Node::ProvisionedService
  ...
  class << self
    ...
    def init(options)
      ...
      @@max_storage_size = options[:max_storage_size]
    ...
    def create(port, name, username, password)
      ...
      provisioned_service.loop_create(@@max_storage_size)
  ...
  def service_script
    # prepare the config file to start vblob
    provisioned_service = self
    vblob_root_dir = "/store/instance"
    ...
    result = config.gsub(34.chr, "\\\"")
    "echo \"#{result}\" > #{config_file}; #{@@nodejs_path} #{@@vblobd_path}/server.js -f #{config_file}"
  end
