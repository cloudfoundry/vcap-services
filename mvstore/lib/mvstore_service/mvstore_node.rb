...
class VCAP::Services::MVStore::Node
  ...
  def initialize(options)
    ...
    @mvstore_restore_path = options[:mvstore_restore_path]
  ...
  def restore(instance_id, backup_file)
    ...
    # Run mvstore_restore
    # Review (maxw): This is just a placeholder at the moment...
    command = "#{@mvstore_restore_path} -u #{username} -p#{password} --port #{port} #{backup_file}"
    output = `#{command}`
    res = $?.success?
    @logger.debug(output)
    raise 'mvstore_restore failed' unless res
    true
  ensure
    conn.close if conn
  end
  ...
  def import_instance(service_credential, binding_credentials, dump_dir, plan)
    ...
    FileUtils.cp_r(File.join(dump_dir, instance_id), @mvstore_log_dir)
    true
  rescue => e
    @logger.warn(e)
    nil
  ...
  def start_instance(provisioned_service)
    ...
    if pid
      ...
      config = @config_template.result(binding) # Review (maxw): Sonic says this is not needed.
      config_path = File.join(dir, "mvstore.conf")
      ...
      cmd = "LD_LIBRARY_PATH=#{ld_library_path} #{@mvstore_path} -s #{data_dir} -d #{web_console_dir} -p #{port}"
  ...
  def mvstore_add_admin(options)
    @logger.info("mvstore_add_admin: #{options}")
    raise "No admin user support in mvstore #{options[:username]}"
  end

  def mvstore_add_user(options)
    @logger.debug("mvstore_add_user: port=#{options[:port]}, db=#{options[:db]}")
    raise "No explicit user management needed in mvstore"
  end

  def mvstore_remove_user(options)
    @logger.debug("mvstore_remove_user: port=#{options[:port]}, db=#{options[:db]}")
    raise "No explicit user management needed in mvstore"
  end
