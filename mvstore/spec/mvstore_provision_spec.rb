...
describe "mvstore_node provision" do
  ...
  it "should allow authorized user to access the instance" do
    EM.run do
      urlbase = "http://#{@resp['username']}:#{@resp['password']}@localhost:#{@resp['port']}/db"
  ...
  it "should not allow unauthorized user to access the instance" do
    EM.run do
      url = "http://#{@resp['username']}:boguspw@localhost:#{@resp['port']}/db?q=#{CGI::escape("SELECT * FROM mv:ClassOfClasses;")}&i=mvsql&o=json"
      begin
        response = RestClient.get url
      rescue Exception => e
        @logger.debug e
  ...
  it "should be able to unprovision an existing instance" do
    EM.run do
      ...
      e = nil
      url = "http://#{@resp['username']}:#{@resp['password']}@localhost:#{@resp['port']}/db?q=#{CGI::escape("SELECT * FROM mv:ClassOfClasses;")}&i=mvsql&o=json"
      begin
        response = RestClient.get url
      rescue Exception => e
        @logger.debug e
