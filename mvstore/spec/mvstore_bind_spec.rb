...
describe "mvstore_node bind" do
  ...
  it "should have valid response" do
    ...
    @resp['host'].should == @bind_resp['host']
  ...
  it "should allow authorized user to access the instance" do
    EM.run do
      url = "http://#{@bind_resp['username']}:#{@bind_resp['password']}@localhost:#{@resp['port']}/db?q=#{CGI::escape("SELECT *;")}&i=mvsql&o=json"
  ...
  it "should not allow unauthorized user to access the instance" do
    EM.run do
      url = "http://#{@bind_resp['username']}:boguspw@localhost:#{@resp['port']}/db?q=#{CGI::escape("SELECT * FROM mv:ClassOfClasses;")}&i=mvsql&o=json"
      begin
        response = RestClient.get url
      rescue => e
      end
