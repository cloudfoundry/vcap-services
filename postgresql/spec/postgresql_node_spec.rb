...
describe "Postgresql node normal cases" do
  ...
  it "should be able to get public schema id and get all user created schemas" do
    EM.run do
      ...
      # use a default user (parent role)
      puts tmp_db
      default_user = VCAP::Services::Postgresql::Node::Provisionedservice.get(tmp_db['name']).bindusers.all(:default_user => true)[0]
      puts default_user.user
      puts default_user.password
