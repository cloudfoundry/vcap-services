...
module VCAP::Services::Serialization::Postgresql
  ...
  class CreateSerializedURLJob < SerializationJob
    ...
    def serialize_db(name)
      ...
      # setup DataMapper
      setup_datamapper(:default, @config['local_db'] )
      # prepare the command
      provisionedservice = Model::Provisionedservice.get(name)
  ...
  class ImportFromURLJob < SerializationJob
    ...
    def import_db_from_url(name, url)
      ...
      setup_datamapper(:default ,@config["local_db"])
      service = Model::Provisionedservice.get(name)
      ...
      default_user = service.bindusers.all( :default_user => true )
