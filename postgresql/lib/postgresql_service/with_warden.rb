...
module VCAP::Services::Postgresql::WithWarden
  ...
  def init_global_connection(instance)
    ...
    @connection_mutex.synchronize do
      @connections[instance.name] = [Time.now.to_i, false]
    end
  end
  ...
  def setup_global_connection(instance)
    ...
    @connection_mutex.synchronize do
      @connections[instance.name] = [Time.now.to_i, conn]
    end
    conn
  end
  ...
  def global_connection(instance=nil, keep_alive=false)
    ...
    if instance
      ...
      time, conn = fetch_global_connection(name)
      if keep_alive && (conn != false && (conn.nil? || connection_exception(conn)))
        ...
        @connection_mutex.synchronize do
          @connections[name] = [Time.now.to_i, conn]
        end
      end
      if conn === false
        @connection_mutex.synchronize do
          @connections.delete(name)
        end if keep_alive && time && (Time.now.to_i - time) > 300
        conn = nil
      end
    end
    conn
  ...
  def kill_long_queries
    pgProvisionedService.all.each do |service|
      conn = fetch_global_connection(service)[1]
      @long_queries_killed += kill_long_queries_internal(conn, postgresql_config(service)['user'], @max_long_tx) if conn
    end
  rescue => e
