      ...
      module Util
        ...
        def grant_write_access_internal(db_connection, service, public_schema_id=nil)
          ...
          unless default_connection
            @logger.warn("Default user failed to connect to database #{name} when granting write access")
            return false
          end
          ...
          service.bindusers.all.each do |binduser|
            ...
            if db_connection_sys_user.nil?
              @logger.warn("Unable to grant write access to #{name} for #{sys_user}") if @logger
            else
        ...
        def reset_db(host, port, vcap_user, vcap_pass, name, service)
          ...
          # should grant create privilege on database to parent role for restoring schema
          default_user = service.bindusers.all(:default_user => true)[0]
          dbconn.query("grant create on database #{name} to #{default_user.user}")
