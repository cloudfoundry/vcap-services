      ...
      module Util
        ...
        def restore_database(name, host, port, user, passwd, dump_file, opts = {} )
          ...
          if s.exitstatus == 0
            ...
            # FIXME it is like a hacker way for postgresql 9.0+
            if s.exitstatus == 1 && e.to_s.index('could not execute query: ERROR:  must be owner of database') && e.to_s.index("CREATE OR REPLACE PROCEDURAL LANGUAGE plpgsql;") && e.to_s.index("WARNING: errors ignored on restore: 1")
              # http://www.postgresql.org/docs/9.0/interactive/sql-createlanguage.html
              # the user may be not the owner of the language plpgsql and could not create or replace the procedural language, just ignore this error.
              # Or we just use the superuser to restore the database?
              opts[:logger].warn("Ignored Error => #{e}") if opts[:logger] && opts[:logger].respond_to?(:warn)
              result = true
            end
          end
