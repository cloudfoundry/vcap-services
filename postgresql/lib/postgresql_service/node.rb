...
class VCAP::Services::Postgresql::Node
  ...
  def restore(name, backup_path)
    ...
    cmd = "#{@gzip_bin} -dc #{path}|" +
      "#{@psql_bin} -h #{host} -p #{port} -U #{user} -d #{name}"
    o, e, s = exe_cmd(cmd)
    if s.exitstatus == 0
      return true
    else
  ...
  def dump_instance(prov_cred, binding_creds, dump_file_path)
    ...
    cmd = "#{@dump_bin} -h #{host} -U #{user} #{name} > #{dump_file}"
    o, e, s = exe_cmd(cmd)
    if s.exitstatus == 0
  ...
  def import_instance(prov_cred, binding_creds_hash, dump_file_path, plan)
    ...
    cmd = "#{@psql_bin} --host=#{host} --user=#{user} -d #{name} < #{import_file}"
    o, e, s = exe_cmd(cmd)
    if s.exitstatus == 0
      return true
    else
